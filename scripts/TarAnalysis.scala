...
import michid.shell.Tar

import scala.io.Source

/** For long format listing (tars | long) */
def long = (path: Path) => (path.owner, path.group, path.size, path.ctime, path.name)

/** For short format listing (tars | short) */
def short = (path: Path) => path.name
